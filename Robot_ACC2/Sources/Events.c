	...
	if(found_band==0){
		CodError = AS1_RecvChar( & anuncio ) ; // Header 0xff
			if (anuncio  == 0xff ) {
			 ...
			 CodError =  AS1_RecvChar( & anuncio ) ; // Header 0x00
			 if (anuncio == 0){
		 ...
		 CodError =  AS1_RecvChar( &n_bytes) ; // Header 0x00
		 found_band = found_band+1;
	}
	else if (found_band == 3){
		...
		if (command == 1){
			...
			anuncio = 0;
			 ...
			 if (found_band == (n_bytes+3)){ // Se lee hasta que se alcance el numero de bytes de trama
				 found_band = 0; // Se termino la lectura del Bloque, se reincia la lectura
				 command = 0;
				 anuncio = 0;
